...
#define FIXED_ANGLE_CORRECTION (.273)  // ***** Replace the value 0.25 with the value you obtained from the Gyro calibration procedure
...
//  float Kp = 2678;//2.010830673979889 * pow(10, 6); //2.010830673979889*pow(10,6);
//  float Ki = 1201;//3.348153635231297 * pow(10, 4);
//  float Ci = 0;
//  float Jp = 0;
//  float Ji = 0;

//  float Kp = 1198.5;//2.010830673979889 * pow(10, 6); //2.010830673979889*pow(10,6);
//  float Ki = 4917.47;//3.348153635231297 * pow(10, 4);
//  float Ci = -126.9445;
//  float Jp = -114.2857;
//  float Ji = -271.7915;
  float Kp = 5256.386684752932;//2.010830673979889 * pow(10, 6); //2.010830673979889*pow(10,6);
  float Ki = 26306.76168312164;//3.348153635231297 * pow(10, 4);
  float Ci = -352.5423503896048;
  float Jp = 465.9548571428571;
  float Ji = -1503.543498777914;
  ...
  v_c_L = v_c_R;//v_d + (Jp*measured_speedL + Ji *distLeft_m) + dist_accum*Ci;
  ...
  if ((cur_time - prev_time) > UPDATE_TIME_MS) {
    ...
    if (angle_rad > -0.1 && angle_rad < 0.1 && ! start_flag)
    {
      ...
      if (start_counter > 30)
      {
        ...
        start_flag = 1;
        buzzer.playFrequency(DIV_BY_10 | 4400, 1000, 15);
